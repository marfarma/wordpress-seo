...
// TODO:
// - performance! [caching images? (transient?)]
...
class WPSEO_Sitemaps {
	...
	function __construct() {
		// always add this action as it needs to be triggered if XML sitemaps are being enabled
		add_action( 'update_option_wpseo', array( $this, 'flush_rewrite' ), 10, 2 );
	...
	function build_root_map() {
		...
		foreach ( get_post_types( array('public' => true) ) as $post_type ) {
			...
			$n = ($count > 1000) ? $count / 1000 : 1;
			for ( $i = 0; $i < $n; $i++ ) {
				$count = ($n > 1) ? $i + 1 : '';
				$this->sitemap .= '<sitemap>' . "\n";
				$this->sitemap .= '<loc>' . home_url( $post_type . '-sitemap' . $count . '.xml' ) . '</loc>' . "\n";
		...
		foreach ( get_taxonomies( array('public' => true) ) as $tax ) {
			...
			$this->sitemap .= '<sitemap>' . "\n";
			$this->sitemap .= '<loc>' . home_url( $tax . '-sitemap.xml' ) . '</loc>' . "\n";
			$this->sitemap .= '<lastmod>' . htmlspecialchars( $date ) . '</lastmod>' . "\n";
			$this->sitemap .= '</sitemap>' . "\n";
		}
	...
	function output() {
		...
		echo $this->sitemap;
		echo "\n" . '<!-- XML Sitemap generated by Yoast WordPress SEO | Built in ' . timer_stop() . ' seconds | ' . memory_get_peak_usage() . '-->';
	}
	 ...
	 */
	function canonical() {
		return ! isset( $GLOBALS['wp']->query_vars['sitemap'] );
	}

	/**
	 * Flush rewrite rules if WP SEO options have been updated to (en|dis)able XML Sitemaps.
	 */
	function flush_rewrite( $old, $new ) {
		if ( ! isset( $old['enablexmlsitemap'], $new['enablexmlsitemap'] ) &&
			( isset( $old['enablexmlsitemap'] ) || isset( $new['enablexmlsitemap'] ) )
		) {
			flush_rewrite_rules();
		}
	}
	...
	function get_last_modified( $post_type ) {
		global $wpdb;
		...
		if ( ! $date )
			$date = $wpdb->get_var( $wpdb->prepare( "SELECT post_modified_gmt FROM $wpdb->posts WHERE post_status = 'publish' AND post_type IN (%s) ORDER BY post_modified_gmt DESC LIMIT 1", $post_type ) );
		if ( $date )
